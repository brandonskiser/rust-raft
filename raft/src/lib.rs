...
impl Node {
    ...
    pub fn tick(&mut self) -> Option<Ready> {
        self.raft.next_ready = Some(ReadyBuilder::new());
...
struct Raft {
    ...
    next_ready: Option<ReadyBuilder>,
...
impl Raft {
    ...
    fn start_election(&mut self) -> Ready {
        ...
        let messages = self.broadcast(MessageRPC::RequestVote(RequestVoteArgs::new(&self)));
        Ready {
            messages,
            ..Default::default()
        }
    ...
    fn tick_leader(&mut self) -> Option<Ready> {
        if self.heartbeat_timed_out() {
            ...
            let messages = self.broadcast(MessageRPC::AppendEntries(AppendEntriesArgs::new(&self)));
            return Some(Ready::builder().with_messages(messages).build());
        } else {
            None
        }
    }
    ...
    fn step_candidate(&mut self, m: &Message) -> Option<Ready> {
        match m.variant {
            MessageRPC::AppendEntries(ref args) => None,
            MessageRPC::RequestVote(ref args) => None,
            MessageRPC::RequestVoteResp(vote_granted) => None,
